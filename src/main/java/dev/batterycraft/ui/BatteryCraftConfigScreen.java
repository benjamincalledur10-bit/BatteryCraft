...
public final class BatteryCraftConfigScreen {
    ...
    public static void open(BatteryCraftConfig config, Runnable afterSave, Supplier<String> sessionSummary) {
        SwingUtilities.invokeLater(() -> {
            ...
            current = build(config, afterSave, sessionSummary);
    ...
    private static JFrame build(BatteryCraftConfig config, Runnable afterSave, Supplier<String> sessionSummary) {
        ...
        JCheckBox enabled = new JCheckBox("Activar BatteryCraft", config.enabled());
        JCheckBox notifications = new JCheckBox("Mostrar avisos", config.notifications());
        JCheckBox indicator = new JCheckBox("Mostrar indicador sobre la barra", config.hudIndicator());
        JCheckBox sodium = new JCheckBox("Integracion con Sodium", config.sodiumIntegration());
        JCheckBox thermal = new JCheckBox("Modo termico experimental", config.thermalMode());
        ...
        general.add(new JLabel("Bateria baja (%)")); general.add(low);
        ...
        addProfileTab(tabs, panels, config, PowerProfile.BATTERY, "Bateria");
        addProfileTab(tabs, panels, config, PowerProfile.LOW_BATTERY, "Bateria baja");
        addProfileTab(tabs, panels, config, PowerProfile.CRITICAL_BATTERY, "Critico");

        JLabel summary = new JLabel(sessionSummary.get());
        summary.setBorder(BorderFactory.createEmptyBorder(8, 12, 8, 12));
        JButton folder = new JButton("Abrir carpeta de configuracion");
        ...
        JButton save = new JButton("Guardar y aplicar");
        save.addActionListener(event -> {
            try {
                ...
                JOptionPane.showMessageDialog(frame, "Configuracion guardada.", "BatteryCraft", JOptionPane.INFORMATION_MESSAGE);
    ...
    private static void addProfileTab(JTabbedPane tabs, Map<PowerProfile, ProfilePanel> panels,
                                      BatteryCraftConfig config, PowerProfile profile, String title) {
        ProfilePanel panel = new ProfilePanel(config.profile(profile));
    ...
    private static final class ProfilePanel extends JPanel {
        ...
        private ProfilePanel(ProfileSettings settings) {
            ...
            clouds = new JCheckBox("Activadas", settings.clouds());
            shadows = new JCheckBox("Activadas", settings.entityShadows());
            row("FPS maximos", fps); row("Distancia de renderizado", render);
            row("Distancia de simulacion", simulation); row("Particulas (0-2)", particles);
            row("Nubes", clouds); row("Sombras de entidades", shadows);
            row("Mezcla de biomas", biomeBlend); row("Distancia de entidades", entityDistance);
        }
